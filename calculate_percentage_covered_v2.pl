...
print OUTPUT "TxID\tday0_mRNA\tday2_mRNA\tday4_mRNA\tday0_PPS\tday2_PPS\tday4_PPS\n";
foreach my $key (keys %PPSCoverage){
    print $key."\n";
    my @values=@{$PPSCoverage{$key}};
    my @mRNA; 
    if (exists $mRNARead{$key}) {
        @mRNA=@{$mRNARead{$key}}
    }else{
        @mRNA=(0,0,0); 
    }
